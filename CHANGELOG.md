...
### ✨ Added
- Playback speed capability discovery via `getSupportedPlaybackSpeeds(deviceUdn)`.
- New playback speed models exposed by generated APIs:
  - `SupportedPlaybackSpeeds`
  - `PlaybackSpeedToken`

### 🔄 Changed
- Improved playback speed developer guidance to recommend querying supported speed tokens before calling `setPlaybackSpeed()`.
- Clarified device-compatibility expectations for playback speed control.

### 📖 Documentation
- Added playback speed usage examples for supported token lookup and fallback behavior.
