...
struct ManufacturerDetails {
  ...
  var manufacturerUri: Url? = nil


  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> ManufacturerDetails? {
    let manufacturer = pigeonVar_list[0] as! String
    let manufacturerUri: Url? = nilOrValue(pigeonVar_list[1])
...
struct ModelDetails {
  ...
  var modelUri: Url? = nil


  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> ModelDetails? {
    let modelName = pigeonVar_list[0] as! String
    let modelDescription: String? = nilOrValue(pigeonVar_list[1])
    let modelNumber: String? = nilOrValue(pigeonVar_list[2])
    let modelUri: Url? = nilOrValue(pigeonVar_list[3])
...
struct DlnaDevice {
  ...
  static func fromList(_ pigeonVar_list: [Any?]) -> DlnaDevice? {
    ...
    let manufacturerDetails = pigeonVar_list[3] as! ManufacturerDetails
    let modelDetails = pigeonVar_list[4] as! ModelDetails
    let ipAddress = pigeonVar_list[5] as! IpAddress
    let port = pigeonVar_list[6] as! NetworkPort
    let presentationUrl: Url? = nilOrValue(pigeonVar_list[7])
    let icons: [DeviceIcon]? = nilOrValue(pigeonVar_list[8])
...
struct PlaybackSpeed {
  ...
  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> PlaybackSpeed? {
...
private class MediaCastDlnaPigeonPigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 143:
      return DeviceIcon.fromList(self.readValue() as! [Any?])
    case 144:
      return ManufacturerDetails.fromList(self.readValue() as! [Any?])
    case 145:
      return ModelDetails.fromList(self.readValue() as! [Any?])
    case 146:
...
private class MediaCastDlnaPigeonPigeonCodecWriter: FlutterStandardWriter {
  override func writeValue(_ value: Any) {
    if let value = value as? TransportState {
      ...
      super.writeByte(142)
      super.writeValue(value.toList())
    } else if let value = value as? DeviceIcon {
      super.writeByte(143)
      super.writeValue(value.toList())
    } else if let value = value as? ManufacturerDetails {
      super.writeByte(144)
      super.writeValue(value.toList())
    } else if let value = value as? ModelDetails {
      super.writeByte(145)
      super.writeValue(value.toList())
...
protocol MediaCastDlnaApi {
  ...
  func getDeviceManually(uri: Url, completion: @escaping (Result<DlnaDevice?, Error>) -> Void)
}
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: MediaCastDlnaApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      setPlaybackSpeedChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let deviceUdnArg = args[0] as! DeviceUdn
        let speedArg = args[1] as! PlaybackSpeed
        api.setPlaybackSpeed(deviceUdn: deviceUdnArg, speed: speedArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
    ...
    /// this will throw an exception if you try to use it on Android
    /// this will return null if the device is not found
    let getDeviceManuallyChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.media_cast_dlna.MediaCastDlnaApi.getDeviceManually\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getDeviceManuallyChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let uriArg = args[0] as! Url
        api.getDeviceManually(uri: uriArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
...
protocol DiscoveryEventsFlutterApiProtocol {
  ...
  func onRendererOffline(deviceUdn deviceUdnArg: DeviceUdn, completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class DiscoveryEventsFlutterApi: DiscoveryEventsFlutterApiProtocol {
  ...
  /// Emitted when a MediaRenderer becomes unavailable on the network.
  func onRendererOffline(deviceUdn deviceUdnArg: DeviceUdn, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.media_cast_dlna.DiscoveryEventsFlutterApi.onRendererOffline\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage([deviceUdnArg] as [Any?]) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
